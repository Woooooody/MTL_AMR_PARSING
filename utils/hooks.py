...
def _evaluate(eval_fn, input_fn, decode_fn, decode_fn_2, path, config):
    graph = tf.Graph()
    with graph.as_default():
        ...
        with tf.train.MonitoredSession(session_creator=sess_creator) as sess:
            while not sess.should_stop():
                ...
                source = [item.tolist() for item in feats["source"]]
                # print(source)

                all_outputs.extend(outputs)

                # all_source.extend(feats)
        ...
        decoded_symbols = decode_fn(all_outputs)        # predict
        decoded_refs = [decode_fn(refs) for refs in all_refs]
        decoded_refs = [list(x) for x in zip(*decoded_refs)]        # target
        print(decoded_refs)
        decoded_source = decode_fn_2(all_source)
        print(decoded_source)

    return bleu.bleu(decoded_symbols, decoded_refs, decoded_source)
...
class EvaluationHook(tf.train.SessionRunHook):
    ...
    def __init__(self, eval_fn, eval_input_fn, eval_decode_fn, decode_fn_source,base_dir,
        ...
        self._eval_decode_fn_2 = decode_fn_source
