...
class Transformer(interface.NMTModel):
    ...
    def get_encoder_out(self, features, mode, params):
    ...
    def get_inference_func2(self, problem=None):
        def encoding_fn(features, params=None):
            if params is None:
                params = copy.copy(self.parameters)
            else:
            ...
            encoder_output = encoding_graph2(features, "infer", params)
            batch = tf.shape(encoder_output)[0]
        ...
        def decoding_fn(features, state, params=None):
            ...
            log_prob, new_state = decoding_graph(features, state, "infer", params, problem=problem)
