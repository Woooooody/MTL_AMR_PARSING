...
def get_training_input(filenames, params, problem='parsing'):
    ...
    with tf.device("/cpu:0"):
        ...
        if problem == 'parsing':
            tgt_table = tf.contrib.lookup.index_table_from_tensor(
                tf.constant(params.vocabulary["parsing_target"]),
                default_value=params.mapping["parsing_target"][params.unk]
            )
        elif problem == 'amr':
            tgt_table = tf.contrib.lookup.index_table_from_tensor(
                tf.constant(params.vocabulary["amr_target"]),
                default_value=params.mapping["amr_target"][params.unk]
            )
